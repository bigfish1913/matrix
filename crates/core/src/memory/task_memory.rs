...
/// Operations for TaskMemory
pub trait TaskMemoryOps {
    ...
    /// For dependency context reading
    fn for_dependency_context(&self) -> String;
}

impl TaskMemoryOps for TaskMemory {
    /// Extract memory from task execution result
    async fn extract_from_result(
        runner: &ClaudeRunner,
        workspace: &Path,
        ...
        let prompt = format!(
            r#"You are a technical writer extracting learnings from a task execution.
...
Return empty object {{}} if nothing important. Output JSON only."#,
...
mod tests {
    ...
    fn test_task_memory_is_empty() {
        ...
        non_empty.learnings.push("Learned".to_string());
